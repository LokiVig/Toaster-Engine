...
namespace ImGuiNET
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        /// </summary>
        public ImGuiController( GraphicsDevice gd, Veldrid.OutputDescription outputDescription, int width, int height )
        {
            _gd = gd;
            _windowWidth = width;
        ...
        public void CreateDeviceResources( GraphicsDevice gd, OutputDescription outputDescription )
        {
            ...
            _vertexBuffer = factory.CreateBuffer( new Veldrid.BufferDescription( 10000, BufferUsage.VertexBuffer | BufferUsage.Dynamic ) );
            _vertexBuffer.Name = "ImGui.NET Vertex Buffer";
            _indexBuffer = factory.CreateBuffer( new Veldrid.BufferDescription( 2000, BufferUsage.IndexBuffer | BufferUsage.Dynamic ) );
            _indexBuffer.Name = "ImGui.NET Index Buffer";
            RecreateFontDeviceTexture( gd );

            _projMatrixBuffer = factory.CreateBuffer( new Veldrid.BufferDescription( 64, BufferUsage.UniformBuffer | BufferUsage.Dynamic ) );
            _projMatrixBuffer.Name = "ImGui.NET Projection Buffer";

            byte[] vertexShaderBytes = LoadEmbeddedShaderCode( gd.ResourceFactory, "imgui-vertex", ShaderStages.Vertex );
            byte[] fragmentShaderBytes = LoadEmbeddedShaderCode( gd.ResourceFactory, "imgui-frag", ShaderStages.Fragment );
            _vertexShader = factory.CreateShader( new Veldrid.ShaderDescription( ShaderStages.Vertex, vertexShaderBytes, gd.BackendType == GraphicsBackend.Metal ? "VS" : "main" ) );
            _fragmentShader = factory.CreateShader( new Veldrid.ShaderDescription( ShaderStages.Fragment, fragmentShaderBytes, gd.BackendType == GraphicsBackend.Metal ? "FS" : "main" ) );
        ...
        private byte[] GetEmbeddedResourceBytes( string resourceName )
        {
            ...
            using ( Stream s = new FileStream( $"resources/shaders/imgui/{resourceName}", FileMode.Open, FileAccess.Read ) )
            {
        ...
        private void RenderImDrawData( ImDrawDataPtr draw_data, GraphicsDevice gd, CommandList cl )
        {
            ...
            if ( totalVBSize > _vertexBuffer.SizeInBytes )
            {
                ...
                _vertexBuffer = gd.ResourceFactory.CreateBuffer( new Veldrid.BufferDescription( (uint)( totalVBSize * 1.5f ), BufferUsage.VertexBuffer | BufferUsage.Dynamic ) );
            }
            ...
            if ( totalIBSize > _indexBuffer.SizeInBytes )
            {
                ...
                _indexBuffer = gd.ResourceFactory.CreateBuffer( new Veldrid.BufferDescription( (uint)( totalIBSize * 1.5f ), BufferUsage.IndexBuffer | BufferUsage.Dynamic ) );
            }
